...
def main():
    ...
    print("=" * 80)
    print("PERT Masked Language Model Pre-training")
    print("=" * 80)
    ...
    if args.processed_data and os.path.exists(args.processed_data):
        ...
        if args.max_packets_per_file:
            print(f"Max {args.max_packets_per_file} packets per PCAP file")
        payloads = load_pcaps_from_dir(args.data_dir, max_packets_per_file=args.max_packets_per_file)
        
        if len(payloads) == 0:
            print("\nError: No valid packets found!")
            print("Please check if PCAP files contain valid traffic data.")
            return
        
        print(f"Loaded {len(payloads)} packets")
    ...
    print(f"\nTraining curve saved to {os.path.join(args.output_dir, 'training_loss.png')}")
